...
export class VlookupFunction extends BaseFunction {
  ...
  protected validate() {
    ...
    this.args[3] = forceBoolean(this.args[3], this.table);
  }
  // @ts-ignore
  protected main(key: any, range: UserTable, index: number, isSorted = true) {
    if (isSorted) {
      let last = -1;
      for (let y = 0; y <= range.numRows(); y++) {
        if (range.get(y, 0)?.value <= key) {
          last = y;
        } else {
          break;
        }
      ...
      if (last !== -1) {
        return range.get(last, index - 1)?.value;
      }
    } else {
      for (let y = 0; y <= range.numRows(); y++) {
        if (range.get(y, 0)?.value === key) {
          return range.get(y, index - 1)?.value;
        }
      }
    }
