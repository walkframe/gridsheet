...
| `minNumRows`, `maxNumRows` | `number` | Row count limits (accessible via `sheet.minNumRows` / `sheet.maxNumRows`) |
| `minNumCols`, `maxNumCols` | `number` | Column count limits (accessible via `sheet.minNumCols` / `sheet.maxNumCols`) |
...
#### `getCell(point: PointType, resolution?: Resolution, raise?: boolean): CellType | undefined`
Retrieves a cell by its coordinates.

```tsx
const cell = table.getCell({ x: 1, y: 1 });
console.log(cell?.value, cell?.style);
```

#### `getCellByAddress(address: Address, resolution?: Resolution, raise?: boolean): CellType | undefined`
...
const evaluated = table.toValueMatrix({ resolution: 'RESOLVED' });
```
...
```tsx
const values = table.toValueObject({ resolution: 'RESOLVED' });
...
```tsx
const policy = table.getPolicy({ y: 1, x: 1 });
```

#### `stringify(props: { point: PointType; cell?: CellType; resolution?: Resolution }): string`
