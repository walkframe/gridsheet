...
Returns the row header cell address for the given row number by prepending `0`. The row header cell is at column 0 of that row and is used to set row-level properties such as `sortFixed`, `filterFixed`, `height`, and `style`.
...
// multiple rows — use .map()
[1, 2, 6].map(rh)  // ['01', '02', '06']
```

```tsx
import { buildInitialCells, rh } from '@gridsheet/react-core';
...
#### `ch(col: string | number): string`

Returns the column header cell address for the given column letter or column index (1-based, same as `point.x`) by appending `0`. The column header cell is at row 0 of that column and is used to set column-level properties such as `width`, `label`, and `style`.

```tsx
import { ch } from '@gridsheet/react-core';

ch('A')  // 'A0'
ch(1)    // 'A0'  (column index 1 = column A)
ch('B')  // 'B0'
ch(2)    // 'B0'

// multiple columns — use .map()
['A', 'B', 'C'].map(ch)  // ['A0', 'B0', 'C0']
[1, 2, 3].map(ch)        // ['A0', 'B0', 'C0']
```

```tsx
import { buildInitialCells, ch } from '@gridsheet/react-core';

buildInitialCells({
  cells: {
    [ch('A')]: { width: 150, label: 'Product' },
    [ch('B')]: { width: 100, label: 'Q1' },
    [ch(3)]:   { width: 100, label: 'Q2' },
  },
})
```
```
...
To control a GridSheet instance programmatically from outside the component, use `sheetRef` and `storeRef`. See [Props Reference](/api-reference/props#sheetref--storeref) for the full API.