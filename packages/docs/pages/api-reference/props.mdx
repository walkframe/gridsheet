...
The `policy` property references a `Policy` instance registered in the hub by its string key. A `Policy` unifies rendering, parsing, serialization, and header-label logic.
...
// In initialCells configuration
{
  'A1': {
    value: 1000,
    policy: 'currency',    // References hub.policies.currency
