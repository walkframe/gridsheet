...
### Header Cell Addressing (`A0`, `01`, `A0:G0`)

To configure individual column or row header cells, append `0` to the address:

- **`A0`** — Column A's header cell only (width, label, style, etc.)
- **`01`** — Row 1's header cell only (height, style, etc.)
- **`A0:G0`** — Range of column header cells (A through G)

```tsx
const initialCells = {
  // Column A header: set width and label
  A0: { width: 150, label: 'Product' },
...
**Note**: `A: { ... }` applies to **all data cells** in column A (rows 1 and below). Use `A0` to target the column A header cell. Similarly, `1: { ... }` applies to all data cells in row 1; use `01` for the row 1 header.
...
|----------|-----------|-------------|
| `width` | Column header keys only (`A0`, `default`) | Column width in pixels |
| `height` | Row header keys only (`01`, `default`) | Row height in pixels |
| `label` | Column header keys only (`A0`) | Custom display text for the column header |

**Note**: `width`, `height`, and `label` are header-only properties. Specifying them on data cell keys like `A`, `1`, or `D3` has no effect — use `A0`/`01` instead.
...
const initialCells = {
  ...
  'A0': { width: 150, label: 'Name' },
  'B0': { width: 200, label: 'Value' },
