...
If you also want the full set of built-in extended functions (from `@gridsheet/functions`), use `useSpellbook` instead — it wraps `useBook` with all extended functions pre-loaded:
