...
- `onChange` is triggered whenever the table changes. Use `getLastChangedAddresses()` to identify which cells were modified.
- `getObject({ addresses })` retrieves only the changed cells efficiently.
- This example logs activity for each granular event (edit, row/column insert/remove) and shows the changed addresses.
