...
│   ├── svelte-core/        # Svelte implementation
...
The Vue and Svelte implementations are wrappers around the preact-core:

- **@gridsheet/vue-core**: Vue.js wrapper around preact-core
- **@gridsheet/svelte-core**: Svelte wrapper around preact-core

When only the wrapper layer is updated, these packages use the format: `MAJOR.MINOR.PATCH-X` where X starts from 0 and increments as integers.
...
- **Wrapper Updates**: Only `vue-core` or `svelte-core` increment the suffix (0, 1, 2, ...)
