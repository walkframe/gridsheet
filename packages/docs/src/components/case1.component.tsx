...
const SparklinePolicyMixin: PolicyMixinType = {
  renderSheet({ value, point }: RenderProps<UserSheet>) {
    ...
    if (values.length === 0) {
      return <span style={{ color: 'var(--gs-muted-fg)' }}>—</span>;
    }
...
export default function SalesDashboard() {
  ...
  // NOTE: styles passed through buildInitialCells are baked once at the initial
  // (pre-hydration, light) render, so a theme-conditional value would freeze to
  // its light branch. Cell tints/borders therefore use theme-INDEPENDENT
  // semi-transparent rgba that reads over both the light and dark surface.
  // Only the outer React chrome (the sheet-name input below) tracks isDark live.
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '36px' }}>
        ...
        <GridSheet
          ...
          initialCells={buildInitialCells({
            ...
            cells: {
              ...
              '6': { style: { borderTop: `3px double ${structBorder}` } },
            },
          })}
          options={{
      ...
      <section>
        ...
        <div className="dashboard-striped">
          <style>{`
          .dashboard-striped .gs-row-even .gs-cell { background-color: rgba(127, 127, 127, 0.08); }
        `}</style>
          <GridSheet
            book={book}
            ...
            initialCells={buildInitialCells({
              ...
              cells: {
                ...
                C: {
                  style: { ...metricValueStyle, color: 'var(--gs-accent)' },
                  alignItems: 'center',
                },
