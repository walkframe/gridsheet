...
import { GridSheet, buildInitialCells, operations, Policy, ThousandSeparatorPolicyMixin, CheckboxPolicyMixin, makeBorder } from '@gridsheet/react-core';
import type { PolicyMixinType, AutocompleteOption, FeedbackType, SelectProps } from '@gridsheet/react-core';
...
export default function BudgetManagement() {
  ...
  // Card surface + heading follow the theme; the tinted data columns use
  // semi-transparent rgba so they read correctly over either surface.
  const cardBg = isDark ? '#0d1117' : 'white';
  const cardFg = isDark ? '#e4e6e9' : '#2c3e50';
  // NOTE: styles baked into buildInitialCells are frozen at the first (light)
  // render and are NOT re-read when the theme flips — so isDark ternaries would
  // freeze there. Use a theme-independent semi-transparent gray that reads on
  // both surfaces for the structural accent borders.
          ...
          initialCells={buildInitialCells({
            ...
            cells: {
              ...
              A: {
                ...
                width: 100,
                style: {
                  borderRight: `double 3px ${structBorder}`,
                },
                policy: 'department',
              },
              B: {
              ...
              'A6:F6': {
                style: {
                  ...makeBorder({
                    bottom: `4px double ${structBorder}`,
                  }),
                },
              },
              '7': {
