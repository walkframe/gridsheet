...
`options` configures per-sheet behavior. Common fields include:

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `sheetHeight` | `number \| string` | auto | Viewport height. A number is pixels; a string is a CSS dimension (e.g. `'80vh'`). |
| `sheetWidth` | `number \| string` | auto | Viewport width. |
| `sheetResize` | `CSSProperties['resize']` | – | Whether the user can resize the sheet. |
| `editingOnEnter` | `boolean` | – | Enter edits the focused cell. |
...
GridSheet is virtualized, so only cells in the visible range are solved. An **off-screen** cell whose formula calls an async function therefore does not fire until scrolled into view. Set `eager: true` to solve every async formula cell after each update regardless of the visible range, so off-screen rows fill in automatically.

It is off by default to preserve virtualization — eager resolution of async cells can be expensive (side effects, API cost), so committing to "resolve everything" is an explicit, per-sheet decision. The flag is scoped to the sheet (not the book), so one shared book can drive both eager and lazy sheets.

#### Usage

**1. Auto-fill the live grid — just set the flag.** No extra code: off-screen async cells fire in the background as the sheet mounts and changes, so scrolling to them shows resolved values without a loading flash.

```tsx
<GridSheet book={book} initialCells={cells} options={{ eager: true }} />
```

**2. Read every resolved value (e.g. to save/export) — await first.** Resolution is asynchronous, so to read off-screen values back you await [`waitForPending()`](/api-reference/sheet) before serializing. This is the one extra step async inherently requires; it is independent of the `eager` flag.

```tsx
const { sheet } = sheetRef.current;
await sheet.waitForPending();      // wait until async cells settle
const all = toValueMatrix(sheet);  // off-screen rows come back resolved too
```
...
Cell data is designed to be JSON-serializable (see [Serialization Design](/development/architecture#serialization-design)), so you can persist the cells to a database or `localStorage` and restore them later by passing them back as `initialCells`. See [Example 5 — Multi-sheet Excel clone](/examples/case5) for a working example with `localStorage` persistence.
...
- **Header cell borders**: Do not set `border-width` on header cells (row 0 / column 0). Header cell dimensions are used to calculate the editor overlay position, so adding border width causes the overlay to shift out of alignment.
