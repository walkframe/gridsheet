...
describe('rsq', () => {
  ...
  describe('normal', () => {
    it('returns 1 for perfect positive correlation', () => {
      const f = new RsqFunction({ sheet, args: [new RangeEntity('A1:A3'), new RangeEntity('B1:B3')] });
      expect(f.call() as number).toBeCloseTo(1, 10);
    });

    it('returns 1 for perfect negative correlation', () => {
      const f = new RsqFunction({ sheet, args: [new RangeEntity('A1:A3'), new RangeEntity('C1:C3')] });
      expect(f.call() as number).toBeCloseTo(1, 10);
    });

    it('returns R² for partial correlation', () => {
      // A=[1,2,3], D=[1,3,2] → r=0.5, R²=0.25
      const f = new RsqFunction({ sheet, args: [new RangeEntity('A1:A3'), new RangeEntity('D1:D3')] });
  ...
  describe('validation error', () => {
    it('throws when ranges have different lengths', () => {
      const f = new RsqFunction({ sheet, args: [new RangeEntity('A1:A2'), new RangeEntity('B1:B3')] });
    ...
    it('throws when a range has constant values (stdev=0)', () => {
      ...
      const f = new RsqFunction({ sheet: table2, args: [new RangeEntity('A1:A2'), new RangeEntity('B1:B2')] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });
  });
});
