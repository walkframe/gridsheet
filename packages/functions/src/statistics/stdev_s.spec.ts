...
describe('stdev.s', () => {
  ...
  describe('normal', () => {
    it('calculates sample standard deviation', () => {
      const f = new StdevSFunction({ table, args: [new RangeEntity('A1:A8')] });
    ...
    it('accepts individual values', () => {
      const f = new StdevSFunction({ table, args: [new ValueEntity(2), new ValueEntity(4)] });
  ...
  describe('validation error', () => {
    it('throws on fewer than 2 values', () => {
      const f = new StdevSFunction({ table, args: [new ValueEntity(5)] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });

    it('throws on zero args', () => {
      const f = new StdevSFunction({ table, args: [] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });
  });
});
