...
describe('stdev.p', () => {
  ...
  describe('normal', () => {
    it('calculates population standard deviation (sqrt of VAR.P)', () => {
      const f = new StdevPFunction({ sheet, args: [new RangeEntity('A1:A8')] });
      // mean=5, population variance=4, stdev=2
      expect(f.call()).toBe(2);
    });

    it('returns 0 for a single value', () => {
      const f = new StdevPFunction({ sheet, args: [new ValueEntity(7)] });
      expect(f.call()).toBe(0);
    });

    it('accepts individual values', () => {
      const f = new StdevPFunction({ sheet, args: [new ValueEntity(2), new ValueEntity(8)] });
      // mean=5, var=((9+9)/2)=9, stdev=3
      expect(f.call()).toBe(3);
    });

    it('is smaller than STDEV.S for same data', () => {
      const fp = new StdevPFunction({ sheet, args: [new RangeEntity('A1:A8')] });
      const fs = new StdevSFunction({ sheet, args: [new RangeEntity('A1:A8')] });
  ...
  describe('validation error', () => {
    it('throws on zero args', () => {
      const f = new StdevPFunction({ sheet, args: [] });
