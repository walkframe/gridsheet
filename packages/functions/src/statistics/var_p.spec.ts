...
describe('var.p', () => {
  ...
  describe('normal', () => {
    it('calculates population variance (divides by n)', () => {
      const f = new VarPFunction({ table, args: [new RangeEntity('A1:A8')] });
      // mean=5, sum of sq dev=32, population var=32/8=4
      expect(f.call()).toBe(4);
    });

    it('returns 0 for a single value', () => {
      const f = new VarPFunction({ table, args: [new ValueEntity(7)] });
      expect(f.call()).toBe(0);
    });

    it('accepts individual values', () => {
      const f = new VarPFunction({ table, args: [new ValueEntity(2), new ValueEntity(8)] });
      // mean=5, var=((2-5)^2+(8-5)^2)/2=9
      expect(f.call()).toBe(9);
    });

    it('differs from VAR.S for same data', () => {
      const fp = new VarPFunction({ table, args: [new RangeEntity('A1:A8')] });
      const fs = new VarSFunction({ table, args: [new RangeEntity('A1:A8')] });
  ...
  describe('validation error', () => {
    it('throws on zero args', () => {
      const f = new VarPFunction({ table, args: [] });
