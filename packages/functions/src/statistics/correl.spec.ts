...
describe('correl', () => {
  ...
  describe('normal', () => {
    it('returns 1 for perfect positive correlation', () => {
      const f = new CorrelFunction({ table, args: [new RangeEntity('A1:A3'), new RangeEntity('B1:B3')] });
      expect(f.call() as number).toBeCloseTo(1, 10);
    });

    it('returns -1 for perfect negative correlation', () => {
      const f = new CorrelFunction({ table, args: [new RangeEntity('A1:A3'), new RangeEntity('C1:C3')] });
      expect(f.call() as number).toBeCloseTo(-1, 10);
    });
  });
  ...
  describe('validation error', () => {
    it('throws when ranges have different lengths', () => {
      const f = new CorrelFunction({ table, args: [new RangeEntity('A1:A2'), new RangeEntity('B1:B3')] });
    ...
    it('throws when a range has constant values (stdev=0)', () => {
      ...
      const f = new CorrelFunction({ table: table2, args: [new RangeEntity('A1:A2'), new RangeEntity('B1:B2')] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });
  });
});
