...
describe('unicode', () => {
  ...
  describe('normal', () => {
    it('returns code point for regular character', () => {
      const f = new UnicodeFunction({ sheet, args: [new ValueEntity('A')] });
      expect(f.call()).toBe(65);
    });

    it('returns code point for first character only', () => {
      const f = new UnicodeFunction({ sheet, args: [new ValueEntity('Apple')] });
      expect(f.call()).toBe(65);
    });

    it('handles surrogate pairs correctly', () => {
      const f = new UnicodeFunction({ sheet, args: [new ValueEntity('🎵')] });
      expect(f.call()).toBe(127925);
    });

    it('converts numbers to string and gets unicode', () => {
      const f = new UnicodeFunction({ sheet, args: [new ValueEntity(123)] });
  ...
  describe('validation error', () => {
    it('missing argument', () => {
      const f = new UnicodeFunction({ sheet, args: [] });
