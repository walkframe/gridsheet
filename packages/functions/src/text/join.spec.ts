...
describe('join', () => {
  ...
  sheet.initialize({ A1: { value: 'apple' }, A2: { value: 'banana' }, A3: { value: 'cherry' } });

  describe('normal', () => {
    it('joins values with a delimiter', () => {
      const f = new JoinFunction({ sheet, args: [new ValueEntity(', '), new ValueEntity('A'), new ValueEntity('B')] });
      expect(f.call()).toBe('A, B');
    });

    it('joins range with a delimiter', () => {
      const f = new JoinFunction({ sheet, args: [new ValueEntity('-'), new RangeEntity('A1:A3')] });
    ...
    it('stringifies numbers correctly', () => {
      const f = new JoinFunction({ sheet, args: [new ValueEntity('-'), new ValueEntity(1), new ValueEntity(2)] });
  ...
  describe('validation error', () => {
    it('missing arguments', () => {
      const f = new JoinFunction({ sheet, args: [new ValueEntity(',')] });
