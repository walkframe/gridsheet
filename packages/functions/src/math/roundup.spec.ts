...
describe('roundup', () => {
  ...
  describe('normal', () => {
    it('rounds up to 0 decimal places by default', () => {
      const f = new RoundupFunction({ table, args: [new ValueEntity(99.44)] });
      expect(f.call()).toBe(100);
    });

    it('rounds up to specified decimal places', () => {
      const f = new RoundupFunction({ table, args: [new ValueEntity(99.44), new ValueEntity(1)] });
  ...
  describe('validation error', () => {
    it('missing argument', () => {
      const f = new RoundupFunction({ table, args: [] });
