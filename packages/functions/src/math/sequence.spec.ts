...
import { Table, FormulaError, ValueEntity, Spilling } from '@gridsheet/react-core';
...
describe('sequence', () => {
  ...
  describe('normal', () => {
    it('generates a single column sequence', () => {
      const f = new SequenceFunction({ table, args: [new ValueEntity(4)] });
      const result = f.call();
    ...
    it('generates a 2D sequence', () => {
      const f = new SequenceFunction({ table, args: [new ValueEntity(3), new ValueEntity(2)] });
      const result = f.call();
      expect(Spilling.is(result)).toBe(true);
    ...
    it('generates a single cell (1x1)', () => {
      const f = new SequenceFunction({ table, args: [new ValueEntity(1), new ValueEntity(1)] });
      const result = f.call();
      expect(Spilling.is(result)).toBe(true);
    ...
    it('generates a single row', () => {
      const f = new SequenceFunction({ table, args: [new ValueEntity(1), new ValueEntity(5)] });
      const result = f.call();
  ...
  describe('validation error', () => {
    it('missing argument', () => {
      const f = new SequenceFunction({ table, args: [] });
    ...
    it('rows less than 1', () => {
      const f = new SequenceFunction({ table, args: [new ValueEntity(0)] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });

    it('columns less than 1', () => {
      const f = new SequenceFunction({ table, args: [new ValueEntity(3), new ValueEntity(0)] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });
  });
});
