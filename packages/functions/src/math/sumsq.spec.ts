...
describe('sumsq', () => {
  ...
  table.initialize({ A1: { value: 3 }, A2: { value: 4 } });

  describe('normal', () => {
    it('calculates sum of squares for values', () => {
      const f = new SumsqFunction({ table, args: [new ValueEntity(3), new ValueEntity(4)] });
      expect(f.call()).toBe(25); // 9 + 16
    });

    it('calculates sum of squares for table range', () => {
      const f = new SumsqFunction({ table, args: [new RangeEntity('A1:A2')] });
      expect(f.call()).toBe(25);
    });

    it('calculates sum of squares combining values and ranges', () => {
      const f = new SumsqFunction({ table, args: [new RangeEntity('A1:A2'), new ValueEntity(5)] });
  ...
  describe('validation error', () => {
    ...
    it('ignores invalid types correctly if ignore: true but might fail in strict ensureNumber?', () => {
      const f = new SumsqFunction({ table, args: [new ValueEntity('invalid')] });
      expect(f.call.bind(f)).toThrow(FormulaError);
    });
  });
});
