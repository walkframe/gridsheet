...
test('select an option in strict accordance with the options', async ({ page }) => {
  await page.goto('http://localhost:5233/iframe.html?id=policy--options&viewMode=story');

  {
    ...
    expect(await a1.locator('.gs-cell-rendered').textContent()).toBe('blue');
  }
...
test('masked words in clipboard', async ({ page, context }) => {
  await page.goto('http://localhost:5233/iframe.html?id=policy--on-clip&viewMode=story');
